...
import HashTable.SpatialHashTable;
...
public class Solver {
    ...
    public void reset() {
        for (int i = 0; i < particles.length; i++) {
            particles[i].setPosition(new Vector2D((float) initX[i],(float)  initY[i]));
    ...
    public void update() {
        for (Particle p : particles) {
            Vector2D vel = p.getVelocity().add(GRAVITY.scale((float) timestep));
            p.setVelocity(vel);
        }
        ...
        for (Particle p : particles) {
            Vector2D pos = p.getPosition().add(p.getVelocity().scale((float) timestep));
            p.setPosition(pos);
            boundary.apply(p);
        }
    }
    ...
    private void gridToParticle() {
        ...
        for (Particle p : particles) {
            ...
            p.setVelocity(new Vector2D((float) newVx,(float)  newVy));
        }
    }
    ...
    private void pushParticlesApart(int iterations) {
    	...
    	SpatialHashTable table = new SpatialHashTable(grid.getCellSize(), particles.length);
